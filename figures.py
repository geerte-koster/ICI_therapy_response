"""
DESCRIPTION:
    Code to reproduce Figures 1, 3, 4, 5 and 7 (and Suppl. Figures 1-4) from the report. Moreover, with the script of figure 7,
    the validation of the best classifiers as produced by the script 'train_and_validate_RF.py' on the witheld Mariathasan data
    and Riaz data is done.
...
def create_boxplot(path_dict, palette, output, figure):
    """
    Funtion that creates boxplot from training and validation scores of RF from the following input:
        :param path_dict: dictionary specifying the paths were the score files can be found
        :param palette: color palette for the different boxplots
        :param output: name that will be used for output file
        :param figure: specification of figure folder to save output file
        :return: dictionary containing the train MCC, validation MCC, accuracy and AUC values, keys are same keys
         as in path_dict
    """
    # create one df with all train MCC, validation MCC and AUC values
    dict_scores = {}
    list_scores = []

    for method in path_dict.keys():
        # create df with the train MCC, validation MCC and AUC values for each of the runs
...
def sign_df(path_dict, dict_scores):
    """
    Function to calculate significance between performance of different runs, used in combination with the
    create_boxplot function.
    :param path_dict: dictionary specifying the paths were the score files can be found
    :param dict_scores: dictionary containing scores of different runs (output of create_boxplot function)
    :return: dataframes containing p-values for the comparisons between the different runs as specified
    in path_dict (Bonferonni corrected)
    """
...
def remove_X(x):
    """
    Function to remove the 'X' prefix which is added to the gene entrez IDs
    """
...
def entrez_to_symbol(x, gene_an):
    """
    Function to convert a list of entrez IDs to gene symbols by using the gene_annotation file (gene_an) as provided by
    Mariathasan et al., 2018. This function returns the list of gene symbols.
    """
    for i, a in enumerate(x):
...
def create_heatmap(df, gene_an, ID_incorrect, ID_correct):
    """
    Function to create a heatmap of a dataframe (df) containing gene expression values. The heatmap only shows gene expression values
    of the overlap in patients that are predicted correct or incorrect using the 15 or 31 gene signature together with TMB and immune
    phenotypes. Function does not return a variable, but save the created heatmap as follows:
    figures/figure4/heatmap_def_{}.eps"
    """
    df = df.copy()
...
def gex_plots(df, overlap, genes, gene_an):
    """
    Function to create boxplots of gene expression values given in the specified dataframe (df). Boxplots will be made for the subgroups
    correctly predicted responders and non-responders, incorrectly predicted responders and non-responders and the remaining responders
    and non-responders. Samples corresponding to these subgroups are specified via the overlap parameter. With the genes parameter, the
    genes for which the boxplots are made a specified. The parameter gene_an is needed to convert entrez IDs to gene symbols.

    Function returns dataframe with p-values of comparisons of the gene expression values of patients in the three different subgroups.
    """

    # add a column to the df which indicates if a sample is always predicted correctly/incorrectly or if that is NA
    df = df.copy()
...
def figure_3(path_dicts, palettes, outputs, figure):
    """
    Inputs for this function are needed for the functions create_boxplot() and sign_df()
    """
...
def figure_5(overlap1, overlap2, gene_an, df_1, df_2, input_genes):
    """
    Function to create heatmaps and gene expression plots of dataframes specified with df_1 and df_2. Heatmaps are made
    based on overlap1, which specifies the overlap in always correct / incorrectly predicted responders/non-responders.

    Gene expression plots are made based on df_2 and overlap2, which specifies the overlap in responders/non-responders
    that are predicted (in)correctly 85% of the times. Gene expression plots are only made for the genes specified via
    the parameter input_genes. Entrez IDs are converted to gene symbols using the parameter gene_an.

    Function returns dataframe with p-values of comparisons of the gene expression values of patients in the three different subgroups,
    which are corrected using and FDR correction with alpha = 0.05.
    """
    ## heatmap of overlap always correct / incorrectly predicted responders/non-responders
    ...
    ## figure b, gene expression values for all four groups
...
def figure_7(path_dict_1, val_dfs, palette, path_dict_2, riaz_dfs, no_IP_dfs, paths_riaz):
    """
    Function to create ensemble classifiers by combining the top 5 classifiers of a specific run in a majority voting system.
    This ensemble classifier is validated on a withheld test dataset and a dataset of melanoma patients (Riaz et al., 2017)

    Input parameters needed:
    - path_dict_1: specifies paths to output of runs done using the 15 + TMB + IP and 31 gene signatures and those signatures
     after addition of the four extra genes
    - palette: necessarry for the create_function that shows train, validation MCC and AUC of classifiers after addition
    of four extra genes to 15 + TMB + IP and 31 gene signatures
    - val_dfs: contains the dataframes with Mariathasan et al. data, which we be used to retrain the final classifiers on
    - path_dict_2: specifies the paths to the trained classifiers that will be validated on Mariathasan data
    - riaz_dfs: contains the dataframes with Riaz et al. data, which we be used to validate the final classifiers on
    - no_IP_dfs: contains the dataframes with Mariathasan et al. data, which we be used to retrain the final classifiers on. These
    are different dfs than the val_dfs, because the dataframes cannot contain immune phenotype as a feature + the values in the
    dataframes are z-score transformed
    - paths_riaz: specifies the paths to the trained classifiers that will be validated on Riaz data
    """
    ...
    # specify which samples are used in test set and which to train classifier
...
def sup_figure_1(path_dict):
    """
    Function that creates Supplementary Figure 1 (with distribution of predictions of responders/non-responders using
    no balancing or SMOTE) from the output of the runs that are specified in the path_dict
    """
    # palette with color for responders (1) and non-responders (0)
    bin_pal = {1: 'cornflowerblue', 0: 'crimson'}

    for run in path_dict.keys():
        path = path_dict[run]
        ID_train = pd.read_csv("{}/predicted_classes_train.csv".format(path))
        ID_test = pd.read_csv("{}/predicted_classes_test.csv".format(path))
        ...
        #fig.savefig(("figures/supl_figure1/fraction_correct_predicted_train_{}.eps").format(run), format='eps')
        ...
        fig = sns.displot(x=ID_test.loc[:, "frac_pred"], hue=ID_test["binaryResponse"], binwidth=0.05, multiple="stack",
                          palette=bin_pal)
        #fig.savefig(("figures/supl_figure1/fraction_correct_predicted_test_{}.eps").format(run), format='eps')

def sup_figure_2(path_dict):
    """ Function that creates Supplementary Figure 2 (boxplots of importance of top 10 features) from the output of the runs
    that are specified in the path_dict parameter
    """
    ...
    for file in path_dict.keys():
        ...
        # sort the features with highest mean and select top 10 to show in boxplot
...
### Figure 5
# overlap when 15 + TMB + IP is compared with 31 + TMB + IP
